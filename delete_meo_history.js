...
/**
 * history_meo.jsonから対象のMEO履歴を削除するスクリプト
 */
function deleteMeoHistory() {
  ...
  try {
    ...
    const filteredHistory = historyData.filter(item => 
      !(item.task.type === 'google' && TARGET_LOCATIONS.includes(item.task.searchLocation) && SALON_NAMES_TO_DELETE.includes(item.task.salonName))
    );
    ...
    console.log(`✅ 処理が完了しました。${deletedCount}件の対象MEO履歴を削除しました。`);
